...
use tp5er\think\auth\permission\exceptions\WildcardPermissionNotProperlyFormatted;
...
class WildcardPermission
{
    ...
    protected function setParts(): void
    {
        ...
        $parts = Collection::make(explode(self::PART_DELIMITER, $this->permission));

        $parts->each(function ($item, $key) {
            $subParts = Collection::make(explode(self::SUBPART_DELIMITER, $item));
